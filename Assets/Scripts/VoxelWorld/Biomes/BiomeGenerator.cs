...
namespace Voxel
{
    public class BiomeGenerator : MonoBehaviour
    {
        ...
        private int GetSurfaceHeightNoise(int x, int z, int chunkHeight)
        {
            ...
            {
                terrainHeight = VoxelNoise.OctavePerlin(x, z, biomeNoiseSettings);
            }
            else
            {
            ...
            terrainHeight = VoxelNoise.Redistribution(terrainHeight, biomeNoiseSettings);
            int surfaceHeight = VoxelNoise.RemapValue01ToInt(terrainHeight, 0, chunkHeight);
